...
{{< image src="echo.png" width="400px" height="300px" caption="echoの権限" >}}

上記から、echoは管理者のみが書き換えも可能、他は読み込みと実行のみが可能です。
実際に、書き換えを行おうとすると以下のようにエラーになります。

{{< image src="" width="400px" height="300px" caption="書き込みの権限エラー" >}}
...
{{< image src="" width="400px" height="300px" caption="リンクファイル" >}}

ここでは、実際にechoを呼ぶecholinkというリンクファイルを作成しています。
...
{{< image src="chmod.png" width="400px" height="300px" caption="chmodのエラー？" >}}
ユーザ管理ってpasswdとかで行っているのか
...
{{< image src="passwd.png" width="400px" height="300px" caption="passwdの権限" >}}

setuidがセットされていると、ファイルの所有者の実行権限が"s"になります。
これにより、ファイル所有者がアクセスできる全てのファイルにアクセスできるようになります。

{{< admonition tip RUIDとEUID>}}
「setuidなどで他のユーザ権限でプロセスを動かした場合、自分自身の権限はどうなるのでしょうか？」

sudoのようにroot権限の利用であれば問題ないですが、setuidで他の一般ユーザの権限を利用する場合などは自分自身の権限が失われると、自分のファイルにアクセスできなくなり、非常に困ります。
